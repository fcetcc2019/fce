	...
	?>
        ...
        <div class="container-fluid">
            ...
            <div class="row-fluid secoes relatorios">
				...
				//$busca = "SELECT id, titulo, unidade, (SELECT u.Nome FROM INF_unidades u WHERE u.IdUO = unidade) as nomeunidade, balcao, (SELECT b.Titulo FROM INF_balcoes b WHERE b.cod = balcao) as nomebalcao, publicado, ativo FROM CAD_enquete ".$where." ORDER BY id DESC";
				$query_graficos = mysql_query($busca, $db_alpha);
				
				$id_enquete = '';
				
				
				while($res_graficos = mysql_fetch_assoc($query_graficos)) {
				
					$id_enquete = $res_graficos['id'];
					
					$sql = "SELECT r.resposta, COUNT(*) AS total FROM CAD_enquete_respostacliente rc INNER JOIN CAD_enquete_resposta r ON r.id = rc.id_resposta INNER JOIN CAD_enquete_pergunta p ON p.id = r.id_pergunta INNER JOIN CAD_enquete e ON e.id = p.id_enquete LEFT JOIN CAD_enquete_respostacliente_outros rco ON rco.id_resposta = r.id AND rco.id_respostacliente = rc.id WHERE e.id = ".$id_enquete." GROUP BY r.resposta";
					...
					<div class="row-fluid">
                        <div id="container<?php echo $id_enquete; ?>" style="min-width: 310px; height: 400px; max-width: 600px; margin: 0 auto"></div>
                        <script type="text/javascript">
                        //$(document).ready(function(){
                            ...
                            // Build the chart
                            Highcharts.chart('container<?php echo $id_enquete; ?>', {
                                chart: {
